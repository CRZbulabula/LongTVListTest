...
public class LongTVList extends TVList {
  ...
  public void sort() {
    if (sortedTimestamps == null || sortedTimestamps.length < size) {
      ...
      sortedTimestamps = new long[arrayNumber][];
      for (int i = 0; i < arrayNumber; i++) {
        sortedTimestamps[i] = new long[ARRAY_SIZE];
      }
    }
    if (sortedValues == null || sortedValues.length < size) {
      int arrayNumber = (int) Math.ceil((float) size / (float) ARRAY_SIZE);
      sortedValues = new Long[arrayNumber][];
      for (int i = 0; i < arrayNumber; i++) {
        sortedValues[i] = new Long[ARRAY_SIZE];
      }
    }
    sort(0, size);
    clearSortedValue();
  ...
  protected void expandValues() {
    values.add(new Long[ARRAY_SIZE]);
  }
